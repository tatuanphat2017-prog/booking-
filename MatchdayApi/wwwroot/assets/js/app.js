...
const MDA = (() => {
  ...
  /**
   * Huy hiệu đội bóng tự tạo (chưa có ảnh logo thật do Admin upload): mỗi đội được
   * gán 1 màu riêng, chọn bằng hash tên đội (cùng 1 tên -> luôn ra cùng 1 màu, không
   * đổi lung tung mỗi lần tải trang). Bảng màu được chọn lọc để hài hoà với theme
   * chung (không dùng màu tuỳ tiện) và đều đã kiểm tra tương phản chữ/nền >=4.5:1.
   * Đây là fallback — nếu team.logoUrl có giá trị thật, nơi gọi sẽ ưu tiên hiện ảnh
   * thay vì huy hiệu tự tạo này (xem hàm crest() ở index.html/match.html).
   */
  const CREST_PALETTE = [
    { bg: '#c9382e', fg: '#ffffff' }, // đỏ
    { bg: '#2f6fd1', fg: '#ffffff' }, // xanh dương
    { bg: '#12793f', fg: '#ffffff' }, // xanh lá
    { bg: '#7c3aed', fg: '#ffffff' }, // tím
    { bg: '#e0932b', fg: '#0c1120' }, // cam
    { bg: '#00697d', fg: '#ffffff' }, // cyan đậm
    { bg: '#c93b8f', fg: '#ffffff' }, // hồng magenta
    { bg: '#c9a227', fg: '#0c1120' }, // vàng đồng
  ];
  function crestTheme(name) {
    if (!name) return { bg: 'var(--bg-elevated)', fg: 'var(--gold-bright)' };
    let h = 0;
  ...
  const ICONS = {
    ...
    drink: '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2h8l-.9 11.2a1.4 1.4 0 0 1-1.4 1.3H6.3a1.4 1.4 0 0 1-1.4-1.3L4 2Z"/><path d="M3.5 5h9"/></svg>',
    check: '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;"><path d="M2.8 8.4 6 11.6l7.2-7.2"/></svg>',
    clock: '<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;"><circle cx="8" cy="8" r="6.2"/><path d="M8 4.6V8l2.6 1.6"/></svg>',
  };
  function icon(name) {
    return ICONS[name] || '';
  }
